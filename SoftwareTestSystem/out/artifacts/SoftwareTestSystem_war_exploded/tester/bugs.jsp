...
<div class="content">
    ...
    <div class="container-fluid">
        <div class="row-fluid">
            ...
            <a class="btn btn-primary" href="bugsOperation.jsp"><i class="icon-plus"></i> Bug</a>
            <!--<button class="btn">Import</button>-->
            <button class="btn" onclick="Export('bugsTable')">导出</button>
            </div>
            <%
            ...
            %>
            <div class="btn-group">
                <form action="/servlet/BugServlet" method="post">
                    <label>
                        <span>Bug编号：</span>
                        <input type="text" style="height: 20px" name="bugNo">
                        <span style="margin-left: 50px">类型：</span>
                        <input type="text" style="height: 20px" name="type">
                        <span style="margin-left: 50px">方案：</span>
                        <input type="text" style="height: 20px" name="method">
            ...
            <div class="well">
                <table class="table" id="bugsTable">
                    ...
                    <tbody>
                    <%
                        ArrayList<Map<String, String>> bugList = null;
                        if (session.getAttribute("bugList") != null) {
                            bugList = (ArrayList<Map<String, String>>) session.getAttribute("bugList");
                            for (int i = 0; i < bugList.size(); i++) {
                                Map<String, String> item = bugList.get(i);
                    ...
                    <tr>
                        ...
                        <td><%=item.get("testerNo")%>
                        </td>
                        <td><%=item.get("testerName")%>
                        </td>
                        <td><%=item.get("developerNo")%>
                        </td>
                        <td><%=item.get("developerName")%>
                        </td>
                        <td><%=item.get("method")%>
                        </td>
                        <td><%=item.get("date")%>
                        ...
                        <td>
                            <%
                                String href = "bugNo=" + item.get("bugNo");
                                href += "&bugName=" + item.get("bugName");
                                href += "&type=" + item.get("type");
                                href += "&testerNo=" + item.get("testerNo");
                                href += "&developerNo=" + item.get("developerNo");
                                href += "&method=" + item.get("method");
                                href += "&date=" + item.get("date");
                            %>
                            <%
                                if (item.get("testerNo").equals(((Person) session.getAttribute("Person")).getNo())) {
                            %>
                            <a href="/tester/bugsOperation.jsp?<%=href%>"><i
                                    class="icon-pencil"></i></a>
                            <a href="/servlet/BugServlet?deleteBug=1&bugNo=<%=item.get("bugNo")%>" data-toggle="modal"
                               onclick="return confirm('确认删除?')"><i class="icon-remove"></i></a>
                            <%
                                }
                            %>
